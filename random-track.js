...
function track(provider='', query = '', data = '') {
    ...
    if (!!platform_filter) {/*/*
 * Nightbot command:
 * !editcom -ul=everyone -cd=5 !randomtrack $(eval track('$(provider)', '$(query)', "$(urlfetch json https:///*
 * Nightbot command:
 * !editcom -ul=everyone -cd=5 !randomtrack $(eval track('$(provider)', '$(query)', "$(urlfetch json https://docs.google.com/spreadsheets/d/15ZGd_KKINKJhqIS56Fy23YhY9DTeglmCoFYkREVFDek/export?exportFormat=tsv)"); $(urlfetch json https://raw.githubusercontent.com/PhilHoff84/broughy1322/master/random-track.js);)
 */
function track(provider='', query = '', data = '') {
...
*/
    ...
    var matching_tracks = tracks.filter(function (track) {
        return track.matches(query.trim());
    });
...
}docs.google.com/spreadsheets/d/15ZGd_KKINKJhqIS56Fy23YhY9DTeglmCoFYkREVFDek/export?exportFormat=tsv)"); $(urlfetch json https://raw.githubusercontent.com/PhilHoff84/broughy1322/master/random-track.js);)
 */
function track(provider='', query = '', data = '') {
    query = normalize(query);
/*
...
*/
    ...
    /* Find all tracks that match the specified criteria */
    var matching_tracks = tracks.filter(function (track) {
        return track.matches('ps4', query.trim());
    });
...
function track(provider='', query = '', data = '') {
    ...
    var aux = parse_query(query);
    var platform_filter = aux[0];
    var type_filter = aux[1];
    return platform_filter + ' | ' + type_filter;
    if (platform_filter === '') {
        return 'which platform? (PS4 / PC / XB1 / 5M)';
    }
